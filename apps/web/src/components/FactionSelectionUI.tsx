...
import { ShieldHalf, Zap, HelpCircle, Cpu, Skull } from "lucide-react"; // Added Cpu, Skull
...
interface FactionInfo {
  ...
  description: string;
  lore: string;
  icon: string; // Now string, will map to Lucide component
...
interface FactionSelectionUIProps {
  currentGHZ: number; 
  onFactionSelected: (factionId: string) => void; 
}
...
export default function FactionSelectionUI({ currentGHZ, onFactionSelected }: FactionSelectionUIProps) {
  ...
  const getFactionIcon = (iconName: string) => {
    // Using icons suggested in the prompt for selection UI
    if (iconName === "Cpu") return <Cpu className="h-12 w-12 mx-auto mb-3 text-blue-500" />;
    if (iconName === "Skull") return <Skull className="h-12 w-12 mx-auto mb-3 text-red-500" />; // Using red for Skull to differentiate
    // Fallback for other icons from factions.json if needed
